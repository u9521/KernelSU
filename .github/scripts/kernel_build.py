...
def arg_parser():
    parser = argparse.ArgumentParser(
        description="Build GKI Kernel for KernelSU.")
    ...
    parser.add_argument("--manager-sign-size",
                        type=str,
                        help="Expected size of the manager signature.")
    parser.add_argument("--manager-sign-hash",
                        type=str,
                        help="Expected sha256 hash of the manager signature.")
    parser.add_argument(
        "--github-env-file",
        type=str,
...
def setup_kernelsu(kernelsu_dir: str, android_kernel_dir: str, debug: bool):
    ...
    symbol_magic(kernelsu_dir, android_kernel_dir)


def symbol_magic(kernelsu_dir: str, android_kernel_dir: str):
    print()
    print("=" * 50)
    print("Symbol magic")
    print("=" * 50)
    common_root = os.path.join(android_kernel_dir, 'common')
    symbol_list = os.path.join(common_root, 'android/abi_gki_aarch64')

    if os.path.exists(symbol_list):
        print("[+] Add KernelSU symbols")
        export_symbol_txt = os.path.join(kernelsu_dir,
                                         'kernel/export_symbol.txt')
        with open(export_symbol_txt, 'r') as f_in:
            symbols = f_in.readlines()
        with open(symbol_list, 'r+') as f_out:
            existing_symbols = f_out.read()
            for symbol in symbols:
                symbol_strip = symbol.strip()
                if symbol_strip and symbol_strip not in existing_symbols:
                    f_out.write(f"  {symbol_strip}\n")
    else:
        print("[+] No abi_gki_aarch64 found, skip adding symbols")
...
def build_avd_kernel(android_kernel_dir: str, arch: str, mgrsign_size: str,
                     mgrsign_hash: str, kernelVersion: str, outpath: str):
    print()
    print("=" * 50)
    ...
    if mgrsign_size and mgrsign_hash:
        build_env["KSU_EXPECTED_SIZE"] = mgrsign_hash
        build_env["KSU_EXPECTED_HASH"] = mgrsign_hash
    ...
    if kernelVersion == "android-16.1-avd_x86_64":
        bazel_command.extend(["--", f"--destdir={outpath}"])
    else:
        bazel_command.extend(["--", f"--dist_dir={outpath}"])
    run_command(bazel_command, cwd=android_kernel_dir, env=build_env)


def build_gki_kernel(android_kernel_dir: str, mgrsign_size: str,
                     mgrsign_hash: str, kernelVersion: str, outpath: str):
    """Build the kernel."""
    print()
    print("=" * 50)
    ...
    if os.path.exists(os.path.join(android_kernel_dir, 'build/build.sh')):
        ...
        if kernelVersion == "android16-6.12":
            ...
            cmd = [
                'tools/bazel', 'run', '--disk_cache=/home/runner/.cache/bazel',
                '--config=fast', '--config=stamp', '--lto=none',
                '//common:kernel_aarch64_dist', '--'
            ]
            cmd.append(f'--destdir={outpath}')
        else:
            cmd.append(f'--dist_dir={outpath}')
        run_command(cmd, cwd=android_kernel_dir, env=build_env)
...
def main(args):
    ...
    managerSignSize = args.manager_sign_size
    managerSignHash = args.manager_sign_hash
    if managerSignSize and managerSignHash:
        print("manager sign was override!")
        print("size:", managerSignSize)
        print("hash:", managerSignHash)
    ...
    if args.buildtype == 'gki':
        ...
        build_gki_kernel(android_kernel_dir, managerSignSize, managerSignHash,
                         kernelVer, buildout)
