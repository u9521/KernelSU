...
docker run --rm \
  ...
  bash -lc '
    ...
    git -C "$AKB_SOURCE_ROOT/common" reset --hard HEAD

    uv run akb tools add-git-safe /workspace -r

    args=(
      uv run /workspace/KernelSU/.github/scripts/kernel_patch.py
      --kernel-source-path "$AKB_SOURCE_ROOT"
    ...
    mv -v $AKB_DIST_DIR/* /workspace/KernelSUbuild
