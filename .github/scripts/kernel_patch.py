...
AVD_X86_PATCHES = {
    "6.6": [
        "6.6-x86-add-indirect-safe-syscall-dispatch.patch",
        "6.6-x86-add-syscall-hardening-cmdline-switch.patch",
    ],
    "6.12": [
        "6.12-x86-add-syscall-hook-support.patch",
    ],
    "6.18": [
        "6.18-x86-add-indirect-safe-syscall-dispatch.patch",
        "6.18-x86-add-syscall-hardening-cmdline-switch.patch",
    ],
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Patch Android kernel sources with KernelSU changes.")
    parser.add_argument("--debug", action="store_true", help="Enable debug features for the kernel.")
    parser.add_argument("--kernel-source-path", type=str, required=True, help="Path to the Android kernel source.")
    parser.add_argument("--ksu-source-path", type=str, required=True, help="Path to the KernelSU source.")
...
def run_command(command: List[str],
    ...
    try:
        ...
        return subprocess.CompletedProcess(command, returncode, stdout=full_output, stderr=None)
...
class KernelPatcher:
    ...
    def get_avd_x86_patch_files(self) -> List[str]:
        """Return extra patch files for supported AVD x86 kernel versions."""
        image_label = self.config.image_label or ""
        if not image_label.startswith("avd-"):
            return []
        if "-x64-" not in image_label and not image_label.endswith("-x64"):
            return []

        for version, patch_files in AVD_X86_PATCHES.items():
            if f"-{version}-" in image_label:
                return patch_files

        return []

    def apply_patch_file(self, patch_path: str) -> None:
        """Apply a git patch file to the kernel common tree."""
        common_dir = os.path.join(self.config.kernel_source_path, "common")
        run_command(["git", "apply", "--check", patch_path], cwd=common_dir)
        run_command(["git", "apply", patch_path], cwd=common_dir)

    def apply_extra_patches(self) -> None:
        """Apply build-specific extra patches when required."""
        patch_files = self.get_avd_x86_patch_files()
        if not patch_files:
            print("[+] No extra kernel patches required for this image label")
            return

        patch_dir = os.path.join(self.config.ksu_source_path, ".github", "patchs")

        print(f"[+] Apply extra AVD x86 kernel patches for {self.config.image_label}")
        for patch_file in patch_files:
            patch_path = os.path.join(patch_dir, patch_file)
            if not os.path.exists(patch_path):
                raise FileNotFoundError(f"Patch file not found: {patch_path}")
            self.apply_patch_file(patch_path)
    ...
    def setup_kernelsu(self) -> None:
        ...
        kernelsu_driver_path = os.path.join(self.config.kernel_source_path, KERNEL_KSU_DRIVER_REL_PATH)
        ...
        if os.path.islink(kernelsu_driver_path):
            ...
            current_target = os.path.abspath(os.path.join(os.path.dirname(kernelsu_driver_path), current_target))
            if current_target != os.path.abspath(expected_driver_target):
                raise RuntimeError(f"Unexpected KernelSU driver symlink target: {current_target}")
            ...
            print(f"[+] Linking KernelSU driver to {self.config.kernel_source_path}/{KERNEL_DRIVERS_REL_PATH}")
            os.symlink(expected_driver_target, kernelsu_driver_path)

        print("[+] Add KernelSU driver to Makefile")
        append_line_once(os.path.join(self.config.kernel_source_path, KERNEL_DRIVERS_MAKEFILE),
                         "obj-$(CONFIG_KSU) += kernelsu/\n")

        print("[+] Add KernelSU driver to Kconfig")
        kconfig_path = os.path.join(self.config.kernel_source_path, KERNEL_DRIVERS_KCONFIG)
        ...
        if self.config.debug:
            ...
            append_line_once(os.path.join(self.config.ksu_source_path, "kernel/Kbuild"),
                             "ccflags-y += -DCONFIG_KSU_DEBUG\n")

        self.apply_extra_patches()
...
def clean_workspace(android_kernel_path: str) -> None:
    ...
    run_command(["git", "commit", "-a", "-m", "Add KernelSU"], cwd=common_dir, env=git_env, check=False)
