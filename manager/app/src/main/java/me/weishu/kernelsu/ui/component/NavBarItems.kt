...
fun NavBarItems() {
    ...
    TopLevelRoute.entries.forEach { route ->
        ...
        val isSelected = navigator.currentTopLevel == route
        val onItemClick = {
            if (!isSelected) {
        ...
        NavigationSuiteItem(
            ...
            selected = isSelected,
            onClick = onItemClick,
            label = { Text(stringResource(route.config.labelId)) },
            icon = {
                TooltipBox(
                    state = tooltipState,
                    ...
                    tooltip = {
                        ...
                        PlainTooltip(modifier = Modifier.padding(toolTipPadding)) {
                            Text(
                                stringResource(route.config.labelId)
                            )
                        }
                    },
                ) {
                    Icon(
                        imageVector = if (isSelected) route.config.selectedIcon else route.config.defaultIcon,
                        contentDescription = stringResource(route.config.labelId)
                    )
                }
            })
    }
