...
/**
 * Breeze's color palette page: owns the state/action wiring so the upstream color
 * palette screen does not have to know about Breeze.
 */
@Composable
fun ColorPaletteEntry() {
    ...
    val actions = ColorPaletteScreenActions(
        ...
        // Required by the shared actions holder, but not surfaced by the Breeze screen yet:
        // the swipe-back gesture is a miuix-nav renderer feature and the pager gesture modes are
        // implemented by miuix's PagerGestureUtils, neither of which Breeze links against.
        onSetEnableSwipeDismiss = viewModel::setEnableSwipeDismiss,
        onSetPagerInterceptionMode = viewModel::setPagerInterceptionMode,
    )

    ColorPaletteScreenBreeze(state, actions)
}
