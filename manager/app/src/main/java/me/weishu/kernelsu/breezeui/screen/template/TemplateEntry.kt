package me.weishu.kernelsu.breezeui.screen.template
...
import me.weishu.kernelsu.ui.screen.template.TemplateActions
import me.weishu.kernelsu.ui.util.isNetworkAvailable
import me.weishu.kernelsu.ui.viewmodel.TemplateViewModel

/**
 * Breeze's app profile template list: owns the state/action wiring so the upstream
 * template screen does not have to know about Breeze.
 */
@Composable
fun TemplateEntry() {
    ...
    val actions = TemplateActions(
        ...
        onCreateTemplate = {
            navigator.push(
                BreezeRoute.TemplateEditor(TemplateViewModel.TemplateInfo(), false)
            )
        },
        onOpenTemplate = { template ->
            navigator.push(
                BreezeRoute.TemplateEditor(template, !template.local)
            )
        },
    )

    AppProfileTemplateScreenBreeze(
        state = uiState,
        actions = actions,
